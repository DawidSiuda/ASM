...
void Filter2(SDL_Surface **image);
void Filter3(SDL_Surface **image);
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        //Paint(screen, image);	// Display the image
...
void Filter(SDL_Surface **screen)
{
    ...
    new_screen = SDL_CreateRGBSurface(0, height, width,24,0,0,0,0); // nowe wymiary
...
//	    old_pixel = 0x00ff0000|old_pixel;	//pobranie
    ...
    *screen = SDL_SetVideoMode( height, width,  24, SDL_SWSURFACE);
...
void Filter2(SDL_Surface **screen)
{
    int width = (*screen)->w;
    int height = (*screen)->h;
    char bytesPerPixel = (*screen)->format->BytesPerPixel;
    ...
    new_screen = SDL_CreateRGBSurface(0,width, height,24,0,0,0,0); // nowe wymiary
    ...
    for (x = 0; x < width; x++) 
    {
        for (y = 0; y < height; y++)
        {
            old_pixel = getpixel(*screen, x, y);	//pobranie
	   ...
	   //  old_pixel = 0x00ff0000|old_pixel;	//pobranie
            ...
            putpixel(new_screen, width-x, y, old_pixel);	//wstawienie w innym miejscu
        }

    }

    
    printf("1) first px:  %08x\n", getpixel(*screen, 50, 50));
    printf("1) second px: %08x\n", getpixel(new_screen, 50, 50));

    *screen = SDL_SetVideoMode( width, height,  24, SDL_SWSURFACE);

    SDL_BlitSurface(new_screen,NULL,*screen,NULL);	// stara powierzchnia nową

    printf("2) first px:  %08x\n", getpixel(*screen, 50, 50));
    printf("2) second px: %08x\n", getpixel(new_screen, 50, 50));
...
void Filter3(SDL_Surface **screen)
{
    int width = (*screen)->w;
    int height = (*screen)->h;
    char bytesPerPixel = (*screen)->format->BytesPerPixel;
    ...
    for (x = 0; x < width; x++) 
    {
	   ...
	   old_pixel = 0x00ffffff-old_pixel;	//pobranie
	   

 
	   if (y == 50 && x == 50) 
	   	printf("pixel: %08x\n", old_pixel);
           putpixel(new_screen, x, y, old_pixel);
        }
